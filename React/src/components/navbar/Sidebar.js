...
function SidebarItem({ navitem, clickSmartPhone, showSidebar }) {
    ...
    return SidebarData.map((item) => {
      return (
        <SidebarItem
          key={item.id}
          navitem={item}
          clickSmartPhone={props.clickSmartPhone}
    ...
    <>
      <div className="navbar">
        <div className="navbar-wrapper">
          ...
          <Link to="/">
            <h1 className="logo">Smart telefoni</h1>
          </Link>
        </div>
      </div>
      {sidebar && (
